...
Bruno Bonatto <bru.bonatto06@gmail.com>
15:13 (há 11 minutos)
para mim
...
aventureiros_ref.set({
    ...
    '001': {
        ...
        'Inventario': ['Espada Forte (SM1)', 'Poção de cura'],
...
for id_jogador, info in dados.items():
    ...
    print(f"🆔 ID: {id_jogador}")

    print(f"🔵 Nome : {info.get('nome')}")

    print(f"🔵 Classes : {info.get('classes')}")

    print(f"🔵 Nível : {info.get('nivel')}")

    print(f"🔵 Vila: {info.get('vila')}")

    print("============================================")

from pprint import pprint

ref = db.reference('/missoes')
...
for id_missao, info in dados.items():
    ...
    if completadores:
        ...
        print(f"✅ Completada por IDs: {', '.join(completadores)}")

    else:

        print("❌ Ainda não completada por nenhum jogador.")
